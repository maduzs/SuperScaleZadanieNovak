...
import { EntityArrayResponseType, TaskConfigService } from '../service/task-config.service';
import { TaskConfigDeleteDialogComponent } from '../delete/task-config-delete-dialog.component';
...
export class TaskConfigComponent implements OnInit {
  ...
  taskConfigs = signal<ITaskConfig[]>([]);
  isLoading = false;
  ...
  protected readonly taskConfigService = inject(TaskConfigService);
  ...
  trackId = (item: ITaskConfig): string => this.taskConfigService.getTaskConfigIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.taskConfigs.set(dataFromBody);
  }

  protected fillComponentAttributesFromResponseBody(data: ITaskConfig[] | null): ITaskConfig[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const taskConfigsNew = this.taskConfigs();
      if (data) {
        for (const d of data) {
          if (taskConfigsNew.some(op => op.id === d.id)) {
            taskConfigsNew.push(d);
          }
        }
      }
      return taskConfigsNew;
    }
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.taskConfigService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
