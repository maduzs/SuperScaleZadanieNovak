...
export class AlertErrorComponent implements OnDestroy {
  ...
  constructor() {
    this.errorListener = this.eventManager.subscribe('superScaleZadanieNovakApp.error', (response: EventWithContent<unknown> | string) => {
      const errorResponse = (response as EventWithContent<AlertError>).content;
      this.addErrorAlert(errorResponse.message);
    });

    this.httpErrorListener = this.eventManager.subscribe(
      'superScaleZadanieNovakApp.httpError',
